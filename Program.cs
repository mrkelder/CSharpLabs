...
namespace ConsoleApp1
{
    ...
    class SquareRootsFinder: DescrimenantFinder, SquareRoots {
        ...
        public void printSquareRoots ()
        {
            ...
            if (D > 0)
            {
                Console.WriteLine("x1 = {0}", (MathF.Pow(b, 2) + Math.Sqrt(D)) / 2 * a);
                Console.WriteLine("x2 = {0}", (MathF.Pow(b, 2) - Math.Sqrt(D)) / 2 * a);
            }
            else if (D == 0)
                Console.WriteLine("x = {0}", (MathF.Pow(b, 2) + Math.Sqrt(D)) / 2 * a);
            else
